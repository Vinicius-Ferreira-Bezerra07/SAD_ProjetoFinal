...
baseSelecionada = st.sidebar.selectbox('Escolha o formato da base', [
                                       'Completa', 'Graduados', 'Abandonos'])
# Selecção de campos da tabala e suas legendas.
legenda = st.sidebar.selectbox('Escolha o campo que deseja consultar a Legenda', [
                               'Martial status', 'Application mode', 'Course', 'Daytime/evening', 'Previous qualification', 'Nacionality', "Mother's and Father's qualification", "Gender"])
...
if (baseSelecionada == 'Completa'):
    ...
    selectColuna = st.selectbox("Escolha a coluna a ser exibida", ["", "Marital status", "Application mode", "Application order", "Course", "Daytime/evening attendance", "Previous qualification", "Nacionality", "Mother's qualification", "Father's qualification", "Displaced", "Educational special needs", "Debtor", "Tuition fees up to date", "Gender", "Scholarship holder", "Age at enrollment", "International", "Curricular units 1st sem (credited)", "Curricular units 1st sem (enrolled)",
                                "Curricular units 1st sem (evaluations)", "Curricular units 1st sem (approved)", "Curricular units 1st sem (grade)", "Curricular units 1st sem (without evaluations)", "Curricular units 2nd sem (credited)", "Curricular units 2nd sem (enrolled)", "Curricular units 2nd sem (evaluations)", "Curricular units 2nd sem (approved)", "Curricular units 2nd sem (grade)", "Curricular units 2nd sem (without evaluations)", "Unemployment rate", "Inflation rate", "GDP"])
    ...
    # Seleção de histograma do Ordem de Aplicação
        ...
        ax2.hist(dataBase[selectColuna], rwidth=0.5,
                 bins=range(8))  # Ajuste o valor de rwidth
        # Ajuste as marcações no eixo x de acordo com o número de bins
    ...
    # Seleção de histograma do Nacionalidade
        ...
        ax2.hist(dataBase[selectColuna], rwidth=0.8,
                 bins=range(21))  # Ajuste o valor de rwidth
    ...
    # Seleção de histograma das Unidades curriculares 1º semestre (creditadas)
        ...
        st.write(
            "Histograma de alunos por Unidades curriculares 1º semestre (creditadas)")
        fig, ax2 = plt.subplots()
    ...
    # Seleção de histograma das Unidades curriculares 1º semestre (matriculado)
        ...
        st.write(
            "Histograma de alunos por Unidades curriculares 1º semestre (matriculado)")
        fig, ax2 = plt.subplots()
    ...
    # Seleção de histograma das Unidades curriculares 1º semestre (avaliações)
        ...
        st.write(
            "Histograma de alunos por Unidades curriculares 1º semestre (avaliações)")
        fig, ax2 = plt.subplots()
    ...
    # Seleção de histograma das Unidades curriculares 1º semestre (aprovado)
        ...
        st.write(
            "Histograma de alunos por Unidades curriculares 1º semestre (aprovado)")
        fig, ax2 = plt.subplots()
        ax2.hist(dataBase[selectColuna], bins=range(23), rwidth=0.8)
        # Define marcações a cada 2 unidades curriculares
    ...
    # Seleção de histograma das Unidades curriculares 1º semestre (sem avaliações)
        ...
        st.write(
            "Histograma de alunos por Unidades curriculares 1º semestre (sem avaliações)")
        fig, ax2 = plt.subplots()
    ...
    # Seleção de histograma das Unidades curriculares 2º semestre (creditadas)
        ...
        st.write(
            "Histograma de alunos por Unidades curriculares 2º semestre (creditadas)")
        fig, ax2 = plt.subplots()
    ...
    # Seleção de histograma das Unidades curriculares 2º semestre (matriculado)
        ...
        st.write(
            "Histograma de alunos por Unidades curriculares 2º semestre (matriculado)")
        fig, ax2 = plt.subplots()
        ax2.hist(dataBase[selectColuna], bins=range(23), rwidth=0.8)
        plt.xticks(range(23))
    ...
    # Seleção de histograma das Unidades curriculares 2º semestre (avaliações)
        ...
        st.write(
            "Histograma de alunos por Unidades curriculares 2º semestre (avaliações)")
        fig, ax2 = plt.subplots()
    ...
    # Seleção de histograma das Unidades curriculares 2º semestre (aprovado)
        ...
        st.write(
            "Histograma de alunos por Unidades curriculares 2º semestre (aprovado)")
        fig, ax2 = plt.subplots()
    ...
    # Seleção de histograma das Unidades curriculares 2º semestre (sem avaliações)
        ...
        st.write(
            "Histograma de alunos por Unidades curriculares 2º semestre (sem avaliações)")
        fig, ax2 = plt.subplots()
        ax2.hist(dataBase[selectColuna], bins=range(11), rwidth=0.7)
        plt.xticks(range(11))
    ...
    # Seleção de histograma do taxa de inflação
        ...
        # Define marcações no eixo X de -8 a 12 com incremento de 2
        ax2.set_xticks(range(-1, 5, 1))
...
# Base apenas com os alunos Graduados
    ...
    # Seleção de histograma do Forma de Aplicação
        ...
        st.write("Histograma de alunos por Aplicação")
        fig, ax2 = plt.subplots()
        ax2.hist(dataGraduate[selectColuna], bins=range(
            18), range=[1, 18], rwidth=0.9)
        plt.xticks(range(19))
    ...
    # Seleção de histograma do Nacionalidade
        ...
        st.write("Histograma de alunos por Nacionalidade")
        fig, ax2 = plt.subplots()
        ax2.hist(dataGraduate[selectColuna], bins=range(21), rwidth=0.8)
        plt.xticks(range(21))
    ...
    # Seleção de histograma do qualificações da mãe
        ...
        st.write("Histograma de alunos por Qualificações da mãe")
        fig, ax2 = plt.subplots()
        ax2.hist(dataGraduate[selectColuna], bins=range(1, 31), rwidth=0.8)
        ax2.set_xticks(range(1, 31))
...
# Base apensa com os alunos Desistentes
    ...
    # Seleção de histograma do Aplicação
        ...
        st.write("Histograma de alunos por Aplicação")
        fig, ax2 = plt.subplots()
        ax2.hist(dataDropout[selectColuna], rwidth=0.8, bins=range(1, 9))
        ax2.set_xticks(range(1, 9))
    ...
    # Seleção de histograma do Forma de Aplicação
        ...
        st.write("Histograma de alunos por Aplicação")
        fig, ax2 = plt.subplots()
        ax2.hist(dataDropout[selectColuna], bins=range(1, 19),
                 range=[1, 18], rwidth=0.9)
        ax2.set_xticks(range(1, 19))
    ...
    # Seleção de histograma do Curso
        ...
        st.write("Histograma de alunos divididos por curso")
        fig, ax2 = plt.subplots()
        ax2.hist(dataDropout[selectColuna], bins=range(1, 18),)
        ax2.set_xticks(range(1, 18))
...
# Legeda Qualificações anteriores
    ...
    st.sidebar.write(
        "19 - Ensino básico 3.º ciclo (9.º/10.º/11.º ano) ou equiv.")
...
# Legeda Qualificações da Mãe e do Pai
    ...
    st.sidebar.write(
        "2 - 1º - Representantes do Poder Legislativo e dos Órgãos Executivos, Diretores, Diretores e Gerentes Executivos")
    st.sidebar.write(
        "3 - 2 - Especialistas em Atividades Intelectuais e Científicas")
    st.sidebar.write("4 - 3 - Técnicos e Profissões de Nível Intermediário")
    st.sidebar.write("5 - 4 – Pessoal administrativo")
    st.sidebar.write(
        "6 - 5 - Serviços Pessoais, Trabalhadores e Vendedores de Segurança e Proteção")
    st.sidebar.write(
        "7 - 6 - Agricultores e trabalhadores qualificados na agricultura, pesca e silvicultura")
    st.sidebar.write(
        "8 - 7 - Trabalhadores qualificados na indústria, construção e artesãos")
    st.sidebar.write(
        "9 - 8 - Operadores de Instalação e Máquinas e Trabalhadores de Montagem")
    ...
    st.sidebar.write(
        "16 - 125 - Especialistas em tecnologias de informação e comunicação (TIC)")
    st.sidebar.write(
        "17 - 131 - Técnicos e profissões de ciência e engenharia de nível intermediário")
    ...
    st.sidebar.write(
        "19 - 141 - Trabalhadores de escritório, secretárias em geral e operadores de processamento de dados")
    st.sidebar.write(
        "20 - 143 - Operadores de dados, contabilidade, estatística, serviços financeiros e operadores relacionados a registros")
    ...
    st.sidebar.write(
        "24 - 153 - Profissionais de cuidados pessoais e similares")
    st.sidebar.write(
        "25 - 171 - Trabalhadores qualificados da construção civil e similares, exceto eletricistas")
    st.sidebar.write(
        "26 - 173 - Trabalhadores qualificados em impressão, fabricação de instrumentos de precisão, joalheiros, artesãos e similares")
    st.sidebar.write(
        "27 - 175 - Trabalhadores de processamento de alimentos, marcenaria, vestuário e outras indústrias e artesanato")
    st.sidebar.write("28 - 191 - trabalhadores de limpeza")
    st.sidebar.write(
        "29 - 192 - Trabalhadores não qualificados na agricultura, produção animal, pesca e silvicultura")
    st.sidebar.write(
        "30 - 193 - Trabalhadores não qualificados na indústria extractiva, construção, indústria transformadora e transportes")
    ...
    st.sidebar.write(
        "10 - Portaria n.º 533-A/99, alínea b2) (Plano Diferente)")
